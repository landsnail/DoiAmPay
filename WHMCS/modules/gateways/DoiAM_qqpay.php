...
function DoiAM_qqpay_form($params) {
    $n1 = $_SERVER['PHP_SELF'];
    if(stristr($n1,'viewinvoice')){
    }else{
        return '<img style="width: 150px" src="'.$systemurl.'/modules/gateways/DoiAM_qqpay/qqpay.png" alt="QQ支付" />';
    }
...
list($price,$rate)= [$params['amount'],1]; 
	...
	$result=da_post("https://api.daimiyun.cn/v2/qqpay/create",$asd);
	$a=$result;
	...
	$code = '<div class="qqpay"><div id="qqpayimg" style="border: 1px solid #AAA;border-radius: 4px;overflow: hidden;margin-bottom: 5px;width: 202px;"><img class="img-responsive pad" src="https://www.daimiyun.cn/qrcode/x.php?qr='.$result['code'].'" style="width: 250px; height: 200px;"></div>';
$code_ajax = '<a href="#" target="_blank" id="qqpayDiv" class="btn btn-success btn-block" style="width: 320px; ">使用手机QQ扫描上面二维码进行支付<br>
	<span class="hidden-lg hidden-md">'.$result['code'].'</span></a></center></div>';
	$code_ajax = $code_ajax.'
<!--微信支付ajax跳转-->
	<script>	
    ...
    function load(){
        ...
        xmlhttp.onreadystatechange=function(){
            if (xmlhttp.readyState==4 && xmlhttp.status==200){
                ...
                if(trade_state=="SUCCESS"){
                    ...
                    document.getElementById("qqpayDiv").innerHTML="支付成功";
                    //延迟 2 秒执行 tz() 方法
        ...
        xmlhttp.open("get","'.$systemurl.'/modules/gateways/DoiAM_qqpay/invoice_status.php?invoiceid='.$invoiceid.'",true);
        //下面这句话必须有
        //把标签/值对添加到要发送的头文件。
    ...
    if(stristr($n1,'viewinvoice')){
        ...
        return '<img style="width: 150px" src="'.$systemurl.'/modules/gateways/DoiAM_qqpay/qqpay.png" alt="微信支付" />';
    }

}

function DoiAM_qqpay_link($params) {
    return DoiAM_qqpay_form($params);
}
